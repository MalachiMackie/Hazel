...
namespace Cheezy
{
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			CZ_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		{
			CZ_PROFILE_SCOPE("glCreateProgram");
			program = glCreateProgram();

			CZ_CORE_ASSERT(shaderSources.size() <= 3, "Maximum of 3 shaders are supported.");
		...
		int index{ 0 };
		{
			CZ_PROFILE_SCOPE("Compile Shader Sources");
		...
		if (isLinked == GL_FALSE)
		{
			...
			CZ_CORE_ERROR("{0}", infoLog.data());
			CZ_CORE_ASSERT(false, "Shader linking failure");
			return;
		}
